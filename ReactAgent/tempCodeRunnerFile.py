...
p_v_x = np.random.randn(num_particles) * 0.5
p_v_y = np.random.randn(num_particles) * 0.5
masses = np.random.uniform(0.5, 1.5, num_particles)

# Define attraction/repulsion between particle types
forces = np.array([
    [ 0.5, -0.3,  0.2, -0.1,  0.1],
    [-0.3,  0.5, -0.2,  0.3, -0.1],
    [ 0.2, -0.2,  0.5, -0.3,  0.2],
    ...
    for i in prange(num_particles):
        ...
        for j in range(num_particles):
            if i != j:
                ...
                if 0 < dist < r:
                    force = forces[particle_types[i], particle_types[j]] * (1 - dist/r) * (masses[j] / masses[i])
...
while True:
    ...
    for i in range(num_particles):
        ...
        if x >= 0 and x < X and y >= 0 and y < Y:
            trail[y, x] = 1  # Add to trail
        color = colour_dict[particle_types[i]]
        cv2.circle(window, (x, y), 2, color, -1)
